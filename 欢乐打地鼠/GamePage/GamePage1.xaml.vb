...
''' </summary>
Public NotInheritable Class GamePage1
    Inherits Page

    Const 孔洞 As Integer = 4
    Const 游戏时间 = 40
    Const 潜伏时间 = 15
    Const 目标分数 = 游戏时间 / (潜伏时间 / 2) * 孔洞 * 100 '每只老鼠100分~~
    Const 失败限制 = 游戏时间 / (潜伏时间 / 2) * 孔洞 * 0.2 '允许错过五分之一
    Dim limate As Double = 失败限制
    ...
    Private Sub Page_Loading(sender As Object, e As RoutedEventArgs) Handles MyBase.Loading
        ...
        For Each amouse In grid1.Children
            If TypeOf amouse Is mouse Then
                ...
                each_mouse.mouse_image_source = New Uri("ms-appx:///images\angle_mouse.png")
                each_mouse.mouse_image_source_hit = New Uri("ms-appx:///images\hit_angle_mouse.png")
                each_mouse.mouse_hit_sound_source = New Uri("ms-appx:///sounds\mouse1.mp3")
    ...
    Private Async Sub GameOver() Handles ContrlBox.TimeArrived
        ...
        For Each el As XmlElement In doc.DocumentElement
            If el.GetAttribute（"关卡名"） = "精灵鼠来袭!" Then
