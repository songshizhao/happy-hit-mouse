...
''' </summary>
Public NotInheritable Class GamePage8
    Inherits Page

    Const 孔洞 As Integer = 9
    Const 游戏时间 = 60
    Const 潜伏时间 = 16
    Const 目标分数 = 游戏时间 / (潜伏时间 / 2) * 孔洞 * 100 '每只老鼠100分~~
    Dim limate As Double = 8
    ...
    Private Sub Page_Loading(sender As Object, e As RoutedEventArgs) Handles MyBase.Loading
        ...
        For Each amouse In grid1.Children
            If TypeOf amouse Is mouse Then
                ...
                each_mouse.mouse_image_source = New Uri("ms-appx:///images\pretty_mouse.png")
                each_mouse.mouse_image_source_hit = New Uri("ms-appx:///images\hit_pretty_mouse.png")
                each_mouse.mouse_hit_sound_source = New Uri("ms-appx:///sounds\mouse2.mp3")
    ...
    Private Async Sub GameOver() Handles ContrlBox.TimeArrived
        ...
        For Each el As XmlElement In doc.DocumentElement
            If el.GetAttribute（"关卡名"） = "注意，注意，这不是演习！" Then
